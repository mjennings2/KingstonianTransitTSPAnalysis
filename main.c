...
int main() {
     ...
     *
    */
    double output [MAX_EXTE][MAX_TRUNC][2];
    int loop1 = 0;
    int loop2 = 0;

    for(loop1 = 0; loop1 < MAX_TRUNC; loop1++) {
        for (int loop2 = 0; loop2 < MAX_EXTE; loop2++) {
            ...
            for (i = 0; i < n; i++) {
                ...
                printf("%d %d\n", temp, temp2);
                fscanf(trafficFile, " %d %d %d %d", &north[temp][temp2], &south[temp][temp2], &east[temp][temp2],
                       &west[temp][temp2]);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if ((int) (extendGreenTimes[i]) > extension) {}
            ...
            for (i = 0; i < 3; i++) {
                ...
                truncationTime += truncations[i] * truncation;
            ...
            for (int i = 0; i < 3; ++i) {
                for (j = 0; j < 12; j++) {
                    secondsSavedOthers += (extensionTime + truncationTime) * east[i][hours[j]] +
                                          (extensionTime + truncationTime) * west[i][hours[j]];
                    secondsLostOthers += (extensionTime + truncationTime) * north[i][hours[j]] +
                                         (extensionTime + truncationTime) * south[i][hours[j]];
                }
            }
            ...
            printf("The buses saved a total of %d h %d m %d s = %d s\n", *(averageTimeSavedBus + 0),
                   *(averageTimeSavedBus + 0),
                   *(averageTimeSavedBus + 0), averageSecondsSavedBus);
            output[extension][truncation][0] = averageSecondsSavedBus;
            ...
            // long long differenceInEmmissions = CURRENT_EMMISSIONS - coTwoTonnes;
            output [extension][truncation][1] = coTwoTonnes;
        }
    }
